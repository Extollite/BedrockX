...
namespace GUI {
	...
	struct GUIButton {
		...
		GUIButton(string && _name, string && _img, bool Url = false) : name(std::move(_name)), img(std::move(_img)), isUrl(Url) {}
	...
	public:
		...
		string content;
		RawFormBinder(string const& cont, std::function<void(WPlayer, DType)>&& _cb, std::vector<string>&& plylist) {
			content = cont;
			cb = std::move(_cb);
	...
	public:
		...
		SimpleFormBinder(shared_ptr<SimpleForm> fm, function<void(WPlayer, DType)>&& cb) : form(fm), callback(cb), IFormBinder(){};
		virtual string_view seralize() {
		...
		virtual void invoke(ServerPlayer& sp, string_view sv) {
			...
			if (sv[0] == 'n') {
				...
				int idx = atoi(sv);
				if (idx >= form->buttons.size()) return;
	...
	public:
		...
		function<void(WPlayer, DType)> callback;
		FullFormBinder(shared_ptr<FullForm> fm, function<void(WPlayer, DType)>&& cb) : form(fm), callback(cb), IFormBinder(){};
		virtual string_view seralize() {
			return form->seralize();
		}
		virtual void invoke(ServerPlayer& sp, string_view sv) {
						...
						try {
							for (int i = 0; i < SZ; ++i) {
								std::visit([&, i](auto& x) {
									...
									if constexpr (std::is_base_of_v<IGUIStrData, std::remove_reference_t<decltype(x)>>) {
										if (isINT) throw 0;
									}
									if constexpr  (std::is_base_of_v<IGUIIntData, std::remove_reference_t<decltype(x)>>) {
										if (!isINT) throw 0;
									}
									if constexpr  (std::is_base_of_v<IGUINullData, std::remove_reference_t<decltype(x)>>) {
										if (!isINT) throw 0;
									}
									if constexpr  (std::is_base_of_v<IGUIDropdownData, std::remove_reference_t<decltype(x)>>) {
										if (!isINT) throw 0;
										int IDX = std::get<int>(res[i]);
										//std::vector<string> options;
										if (IDX >= x.options.size()) throw 0;
	...
	template<typename T,
		typename std::enable_if<std::is_base_of_v<IFormBinder,T>,int>::type=0>
