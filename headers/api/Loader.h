...
#  define VA_EXPAND(...) __VA_ARGS__
template <CHash> struct THookTemplate;
template <CHash> extern THookRegister THookRegisterTemplate;

# define _TInstanceHook(class_inh, pclass, iname, sym, ret, ...)                                                      \
    template <> struct THookTemplate<do_hash(iname)> class_inh {                                                          \
...
#  define _TStaticHook(pclass, iname, sym, ret, ...)                                                                   \
    template <> struct THookTemplate<do_hash(iname)> pclass {                                                             \
